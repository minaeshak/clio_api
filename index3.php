...
if(isset($_POST['Generate']))
{
  ...
  $URL = "https://eu.app.clio.com/api/v4/tasks.json?fields=matter{id,display_number}&query=Prepare%20Documentation%20Abm&status=pending&created_since=".$start_date."T00:00:00";

  $matters_1 = json_decode(send_request($URL,'GET'),true);

  $URL=$matters_1['meta']['paging']['next'];


  $matters_2 = json_decode(send_request($URL,'GET'),true);
  //$matters_1 = array_merge($matters_1,$matters_2);

  $matter_numbers = sizeof($matters_1['data']);
  $matters_2_numbers = sizeof($matters_2['data']);
...
$URL = "https://eu.app.clio.com/api/v4/custom_fields.json?fields=id,name&query=ImageSource";
$result_cf= json_decode(send_request($URL,'GET'),true);
...
$URL = "https://eu.app.clio.com/api/v4/custom_fields.json?fields=id,name&query=AdditionalUsage";
$result_cf= json_decode(send_request($URL,'GET'),true);
$Additional_Usage=$result_cf['data']['0']['id'];

$URL = "https://eu.app.clio.com/api/v4/custom_fields.json?fields=id,name&query=Additional%20Info%20CM";
$result_cf= json_decode(send_request($URL,'GET'),true);
...
for ($k=$i; $k < $matter_2_numbers ; $k++) {

  //$matter_ids_disp[$i] = array("id" => $result['data'][$i]['matter']['id'] , "display" => $result['data'][$i]['matter']['display_number']);
  // $URL = "https://eu.app.clio.com/api/v4/matters.json?fields=display_number,pending_date&query=".$matters_1["data"][$i]["matter"]["display_number"]."&status=pending&pending_date=\"<2019-03-18\"&pending_date=<=\"2019-03-19\"";

  $URL = "https://eu.app.clio.com/api/v4/matters.json?fields=display_number,pending_date,custom_field_values{field_name,value}&query=".$matters_2["data"][$k]["matter"]["display_number"]."&status=pending&pending_date=>".$start_date."&custom_field_ids[]=".$URLL."&custom_field_ids[]=".$Image."&custom_field_ids[]=".$Imprint."&custom_field_ids[]=".$Website."&custom_field_ids[]=".$Additional_Usage."&custom_field_ids[]=".$CM_info	;

  $json_data = send_request($URL,'GET');
  sleep(1);
  //print_r($json_data);
  $result_matter_2 = json_decode($json_data,true);
  if (empty($result_matter_2['data']) == false && $result_matter_2['data']['0']['pending_date'] != $now_date) {
  $result[$k] = array("disp" => $result_matter_2['data']);
    ...
    $final_result[$k]=array("Matter-ID" => $result[$k]['disp']['0']['display_number'] ,
    "Pending Date" => $result[$k]['disp']['0']['pending_date']);

    for ($j=0; $j < 6 ; $j++) {

      if($result[$k]['disp']['0']['custom_field_values'][$j]['field_name']=="URL" )
      {$final_result[$k]['Custom Field: URL']= $result[$k]['disp']['0']['custom_field_values'][$j]['value'];}
    ...
    for ($j=0; $j < 6 ; $j++) {
      ...
      if($result[$k]['disp']['0']['custom_field_values'][$j]['field_name']=="Image" )
      {$final_result[$k]['Custom Field: Image']= $result[$k]['disp']['0']['custom_field_values'][$j]['value'];}
    ...
    for ($j=0; $j < 6 ; $j++) {
      ...
      if($result[$k]['disp']['0']['custom_field_values'][$j]['field_name']=="Imprint" )
      {$final_result[$k]['Custom Field: Imprint']= $result[$k]['disp']['0']['custom_field_values'][$j]['value'];}
    ...
    for ($j=0; $j < 6 ; $j++) {
      ...
      if($result[$k]['disp']['0']['custom_field_values'][$j]['field_name']=="Website" )
      {$final_result[$k]['Custom Field: Website']= $result[$k]['disp']['0']['custom_field_values'][$j]['value'];}
    ...
    for ($j=0; $j < 6 ; $j++) {
      ...
      if($result[$k]['disp']['0']['custom_field_values'][$j]['field_name']=="Additional Usage" )
      {$final_result[$k]['Custom Field: Additional Usage']= $result[$k]['disp']['0']['custom_field_values'][$j]['value'];}
    ...
    for ($j=0; $j < 6 ; $j++) {
      ...
      if($result[$k]['disp']['0']['custom_field_values'][$j]['field_name']=="CM info" )
      {$final_result[$k]['Custom Field: CM info']= $result[$k]['disp']['0']['custom_field_values'][$j]['value'];}
...
//print_r ($ar);
  ...
  $URL = "https://eu.app.clio.com/api/v4/matters/".$id.".json";
